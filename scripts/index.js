  ...
  if (user) {
    ...
    db.collection('users').doc(user.uid).get().then(doc => {
      const html = `
        ...
        <div class="pink-text">${user.admin ? 'Teacher' : 'Student'}</div>
      `;
  ...
  if (data.length) {
    ...
    data.forEach(doc => {
      ...
      if (guide.filepath && guide.filepath.includes(".mp4")) {
        ...
        var media = "<img src =" + guide.filepath + "/>"
      }
      const li = `
        <li>
          <h2 class="collapsible-header red lighten-1"> ${guide.title} - Date : ${new Date(guide.timestamp).toDateString()}</h1>
          <div class="collapsible-body red lighten-2"> 
            <p>${guide.content}</p> 
